...
export function getColorPalette(
    ...
    const isAttributePieChart = (isPieOrDonutChart(type) || (type === FUNNEL_CHART))) && afm.attributes && afm.attributes.length > 0;
...
export function getSeriesItemData(
    ...
    return seriesItem.map((pointValue, pointIndex, arr) => {
        ...
        if (stackByAttribute) {
            ...
            measureIndex = 0;
        } else if (( isPieOrDonutChart(type) || (type=== FUNNEL_CHART)|| (type=== WATERFALL_CHART)) && !viewByAttribute) {
        ...
        if (isPieOrDonutChart(type) || (type=== FUNNEL_CHART)) {
...
export function generateTooltipFn(viewByAttribute, type) {
    ...
    return (point) => {
        ...
        } else if (isPieOrDonutChart(type) || (type=== FUNNEL_CHART)|| (type=== WATERFALL_CHART)) {
